...
function EventViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<EventInterface>(
    ...
    () =>
      getEventById(id, {
        relations: ['user', 'booking_event_participation_rewardsTobooking'],
      }),
  );
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                {hasAccess('booking', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Booking Event Participation Rewards Tobooking"
                    text={
                      <Link
                        as={NextLink}
                        href={`/bookings/view/${data?.booking_event_participation_rewardsTobooking?.id}`}
                      >
                        {data?.booking_event_participation_rewardsTobooking?.booking_time}
                      </Link>
                    }
                  />
                )}
