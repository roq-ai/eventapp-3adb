...
import { getVirtualBuddies } from 'apiSdk/virtual-buddies';
import { VirtualBuddyInterface } from 'interfaces/virtual-buddy';
import { getEventLocations } from 'apiSdk/event-locations';
import { EventLocationInterface } from 'interfaces/event-location';
import { getBookings } from 'apiSdk/bookings';
import { BookingInterface } from 'interfaces/booking';

import { UserPreferencesListPage as ShowCaseList } from 'pages/user-preferences';

function DashboardPage() {
  const { session } = useSession();

  const virtualBuddyStat = useSWR<PaginatedInterface<VirtualBuddyInterface>>(
    () => `/virtual-buddies`,
    () => getVirtualBuddies({ limit: 1, offset: 0 }),
  );

  const eventLocationStat = useSWR<PaginatedInterface<EventLocationInterface>>(
    () => `/event-locations`,
    () => getEventLocations({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: virtualBuddyStat?.data?.totalCount,
      isLoading: virtualBuddyStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Event Locations',
      result: eventLocationStat?.data?.totalCount,
      isLoading: eventLocationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Bookings',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`user-preferences`} passHref legacyBehavior>
                  <Button
