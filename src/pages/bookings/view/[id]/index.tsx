...
function BookingViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<BookingInterface>(
    ...
    () =>
      getBookingById(id, {
        relations: ['user', 'event_booking_event_idToevent'],
      }),
  );
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                {hasAccess('event', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Event Booking Event Id Toevent"
                    text={
                      <Link as={NextLink} href={`/events/view/${data?.event_booking_event_idToevent?.id}`}>
                        {data?.event_booking_event_idToevent?.name}
                      </Link>
                    }
                  />
                )}
